...
#- we set the seed once at the beginning of each lambda experiment (before the loop starts) as suggested by
#  Morris et al in "Using simulation studies to evaluate statistical methods"
#- the authors also suggest to store rng state before and after the single experiments for diagnostic purposes.
...
for (i in 1:30) { # repeat 30 times
  ...
  createMBOrunHypEll4d_noisy(lambda = 1, run = i) # create the MBO run & store it
...
# store the optimization path data frame (opdf) of each run in a list and then bind the elements of the list in a single data frame
#lambda1
opdf_lambda_1 = list()
for (i in 1:30) { #loop over all bo runs with lambda1
  rds.file = paste0(getwd(),sprintf("/dissertation/analysis/hyper_ellipsoid/mbo_runs_data/lambda_1/lambda_1_run_%i.rds", i))
...
opdf_lambda_10 = list()
for (i in 1:30) {
  rds.file = paste0(getwd(),sprintf("/dissertation/analysis/hyper_ellipsoid/mbo_runs_data/lambda_10/lambda_10_run_%i.rds", i))
  mbo = readRDS(rds.file)
  opdf = as.data.frame(mbo$opt.path)
...
# bind the results of both lambda together to one big data frame
