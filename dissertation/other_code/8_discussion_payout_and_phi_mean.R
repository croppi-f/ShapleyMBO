...
###########################################################
# payout paths for hyper ellipsoid (he)
# load the results
shapley.he = readRDS(paste0(getwd(),"/dissertation/analysis/hyper_ellipsoid/shapley_runs_data/shapley1e3.rds"))
# compute the average prediction for each lambda and iter
...
# 1) plot actual and the average predictions with facets
...
# ribbon is centered around zero (avg prediction subtracted)
...
#  MLP tuning example with the original or actual SM and the final SM to see if results
#  change
#load the data
shapley.mlp = readRDS(paste0(getwd(), "/dissertation/analysis/mlp/shapley_run_data/shapley_phoneme_2e4.rds"))
...
#- for that we have to explain the proposal with iml::Shapley() and it is important
#  that we recreate the same Shapley Object
#load mbo results
mbo.mlp = readRDS(paste0(getwd(),"/dissertation/analysis/mlp/mbo_run_data/mbo_phoneme_lambda_1.rds"))
...
samples = ParamHelpers::generateDesign(n = 7000, par.set = mbo.mlp$opt.path$par.set, fun = lhs::randomLHS) # same n
...
S = iml::Shapley$new(
  predictor = P, x.interest = X[1, ], sample.size = 20000 # same sample size
)
